...
func set_text(new_number):
	number = new_number
	$Sprite/Number/Label.text = str(number)
...
func update_size(size, tile_size):
	...
	set_size(new_size)
	$Sprite/Number.set_size(new_size)
	$Sprite/Number/ColorRect.set_size(new_size)
	$Sprite/Number/Label.set_size(new_size)
